<template>
  <div id="chart7" style="height: 280px"></div>
</template>
<script>
  ...
  export default {
    ...
    data: function () {
      return {
        dataModel : {
          ...
          matterCount: [], //事项数量
          workGuideCount: [], //办事指南
          onlineApplicationCount: [], //全程网申量
          downToCurrentCount: [], //事项下沉（下沉本级）
          currentToDownCount: [], //事项下沉（本级下放）
        },
        showLoading:this.param7,
        indexNames:  ['matterCount', 'workGuideCount', 'onlineApplicationCount', 'downToCurrentCount', 'currentToDownCount'],
        indexChineseNames :  ['事项数量', '办事指南', '全程网申量', '事项下沉(下放本级)', '事项下沉(本级下放)'],
        lineWidth : 2
      }
    },
    computed: {
    ...
    methods: {
      ...
      rendom(){
        ...
        this.updateChartSeven(); //加载数据更新图表
      }
    },
    watch: {
